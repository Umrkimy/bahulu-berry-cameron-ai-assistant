...
export default function AIUsage() {
  ...
  const serviceSpend = data?.by_source.map((item) => `${item.source === "WHATSAPP_RAG" ? "WhatsApp RAG" : "Operations Copilot"}: US$${item.estimated_cost_usd.toFixed(4)} / $${item.budget_usd.toFixed(2)}`).join(" · ");

  return <Stack gap="lg">
    <PageHeader title="AI Usage & Budget" description={serviceSpend ?? "Separate estimated caps for Operations Copilot and draft-only WhatsApp RAG, for the current Malaysia calendar month."} />
    {data ? <>
