...
export default function Updates() {
  ...
  return <Stack gap="xl">
    ...
    <Card withBorder p="lg">
      ...
      {alerts.isError ? <Card withBorder p="md"><Text c="red" size="sm" role="alert">{getApiError(alerts.error).message}</Text><Button mt="sm" size="compact-xs" variant="light" onClick={() => void alerts.refetch()}>Try again</Button></Card> : alerts.isLoading ? <Text c="dimmed" py="md" role="status">Loading live issues…</Text> : liveAlerts.length ? <Stack gap="sm">{liveAlerts.map((item) => <AlertItem key={item.id} item={item} />)}</Stack> : <Card withBorder radius="md" p="lg" bg="var(--mantine-color-gray-0)"><Group gap="sm"><ThemeIcon color="green" variant="light" radius="xl"><IconCheck size={18} /></ThemeIcon><div><Text fw={600}>Everything is under control</Text><Text size="sm" c="dimmed">There are no live operational issues right now.</Text></div></Group></Card>}
    </Card>
    <Divider />
    <Stack gap="md">
