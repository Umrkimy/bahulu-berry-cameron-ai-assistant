...
export default function Team() {
  ...
  const openEdit = useCallback((member: TeamMember) => { setEditing(member); form.setValues({ username: member.username, email: member.email, password: "", role: member.role, is_active: member.is_active }); setOpened(true); }, [form]);
  const changeActive = useCallback((member: TeamMember) => modals.openConfirmModal({ title: member.is_active ? "Deactivate account?" : "Reactivate account?", children: <Text size="sm">{member.is_active ? `${member.username} will no longer be able to sign in.` : `${member.username} will be able to sign in again.`}</Text>, labels: { confirm: member.is_active ? "Deactivate" : "Reactivate", cancel: "Cancel" }, confirmProps: { color: member.is_active ? "red" : "green" }, onConfirm: () => void updateTeamMember(member.id, { is_active: !member.is_active }).then(async () => { await queryClient.invalidateQueries({ queryKey: ["team"] }); notifications.show({ title: member.is_active ? "Account deactivated" : "Account reactivated", message: `${member.username}'s access was updated.`, color: "green" }); }).catch((error) => notifications.show({ title: "Unable to update account", message: getApiError(error).message, color: "red" })) }), [queryClient]);
